...
const _corePackages = <_PackageRef>[
  _PackageRef(
    pub: 'composable_core',
    path: 'packages/composable_core',
  ),
  _PackageRef(
    pub: 'composable_network',
    path: 'packages/composable_network',
  ),
];

const _implementedModules = <String, _ModuleBinding>{
  'offline': _ModuleBinding(
    importPath: 'package:composable_offline/composable_offline.dart',
    className: 'ComposableOfflineModule',
    hasBootstrap: true,
  ),
  'cache': _ModuleBinding(
    importPath: 'package:composable_cache/composable_cache.dart',
    className: 'ComposableCacheModule',
    hasBootstrap: false,
  ),
};
...
void main() {
  ...
  File('${root.path}/$_registrarsFile')
      .writeAsStringSync(_generateRegistrarsDart(root.path, flags));
...
void _syncPubspecDependencies(String rootPath, Map<String, dynamic> packages) {
  ...
  final deps = <_PackageRef>[..._corePackages];
...
String _generateRegistrarsDart(String rootPath, Map<String, bool> flags) {
  final imports = <String>{};
  for (final entry in _implementedModules.entries) {
    final packageDir = _optionalPackages[entry.key]?.path;
    final enabled = flags[entry.key] ?? false;
    if (enabled &&
        packageDir != null &&
        Directory('$rootPath/$packageDir').existsSync()) {
      imports.add(entry.value.importPath);
    }
  }
  ...
  for (final key in sortedKeys) {
    ...
    final binding = _implementedModules[key];
    final packageDir = _optionalPackages[key]?.path;
    final enabled = flags[key] ?? false;
    final implemented = binding != null &&
        enabled &&
        packageDir != null &&
        Directory('$rootPath/$packageDir').existsSync();
    ...
    if (implemented && binding.hasBootstrap) {
      buffer.writeln('      bootstrap: _bootstrap${_toPascalCase(key)},');
    }
  ...
  for (final key in sortedKeys) {
    final registerFn = '_register${_toPascalCase(key)}';
    final bootstrapFn = '_bootstrap${_toPascalCase(key)}';
    final binding = _implementedModules[key];
    final packageDir = _optionalPackages[key]?.path;
    final enabled = flags[key] ?? false;
    final implemented = binding != null &&
    ...
    if (implemented) {
      buffer
        ...
        ..writeln('    final config = sl<ComposableCoreConfig>();')
        ..writeln(
          '    await ${binding.className}(config: config).register(sl);',
        )
        ..writeln('  }')
        ..writeln();

      if (binding.hasBootstrap) {
        buffer
          ..writeln('  static Future<void> $bootstrapFn(GetIt sl) async {')
          ..writeln('    final config = sl<ComposableCoreConfig>();')
          ..writeln(
            '    await ${binding.className}(config: config).bootstrap(sl);',
          )
          ..writeln('  }')
          ..writeln();
      }
